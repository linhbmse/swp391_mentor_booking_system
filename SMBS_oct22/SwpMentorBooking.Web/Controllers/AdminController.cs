...
namespace SwpMentorBooking.Web.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        //    _context = context;
        //}
        public AdminController(IUnitOfWork unitOfWork, IUtilService utilService, ApplicationDbContext context)
        {
        ...
        // GET: Show list of pending requests
        //public IActionResult Requests(string status = "pending")
        //{
        ...
        //// POST: Approve or reject the request
        //[HttpPost]
        //public async Task<IActionResult> ProcessRequest(int id, string action)
        //{
        //    var request = _context.Requests.FirstOrDefault(r => r.Id == id);
        //    if (request == null)
        //    {
        ...
        //}
        // GET: Show all requests for admin
        [HttpGet("manage-requests")]
        public IActionResult ManageRequests()
        {
            var allRequests = _context.Requests
                .Include(r => r.Leader)  // Assuming Leader is a navigation property for Student
                .ThenInclude(s => s.User)  // Include User (Leader's details)
                .Include(r => r.Leader.Group)  // Include the Leader's Group
                .ThenInclude(g => g.Wallet)  // Include Group's Wallet
                .ToList();

            return View("~/Views/Admin/ManageRequests.cshtml", allRequests);
        }

        // POST: Process a specific request (approve/reject with custom slot count)
        [HttpPost("process-request")]
        public async Task<IActionResult> ProcessRequest(int id, string action, int slotCount = 10)
        {
            var request = _context.Requests
                .Include(r => r.Leader)
                .ThenInclude(l => l.Group)
                .ThenInclude(g => g.Wallet)
            ...
            {
                request.Status = "approved";
                // Add slots/points to the group's wallet
                if (request.Leader.Group != null && request.Leader.Group.Wallet != null)
                {
                    request.Leader.Group.Wallet.Balance += slotCount;
                }
            }
            else if (action == "reject")
            {
